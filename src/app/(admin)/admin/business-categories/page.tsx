...
import '../job-categories/Categories.css';
...
export default function BusinessCategoriesPage() {
  ...
  const fetchCategories = async () => {
    try {
      setLoading(true);
      const res = await api.get('/admin/categories/business');
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (editingCategory) {
        await api.put(`/admin/categories/business/${editingCategory.id}`, { name });
      } else {
        await api.post('/admin/categories/business', { name });
      }
      setShowModal(false);
  ...
  const handleToggle = async (id: string, currentStatus: boolean) => {
    try {
      await api.patch(`/admin/categories/business/${id}/status`, { isActive: !currentStatus });
      fetchCategories();
    } catch (err: any) {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await api.delete(`/admin/categories/business/${id}`);
      fetchCategories();
    } catch (err: any) {
  ...
  return (
    <div className="categories-page">
      <PageHeader
        ...
        subtitle="Manage the list of available business categories."
        breadcrumbs={[{ label: 'Home', path: '/admin' }, { label: 'Business Categories' }]}
      />

      <div className="admin-card" ref={contentRef}>
        <div className="card-header d-flex justify-content-between align-items-center">
          <h3><MdCategory className="icon-mr" /> Business Categories ({filteredCategories.length})</h3>
