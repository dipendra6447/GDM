...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    return NextResponse.json({ success: true, message: `Employer ${isActive ? 'activated' : 'deactivated'}` });
  } catch (error: any) {
    if (error.message === 'Unauthorized') return NextResponse.json({ success: false, message: 'Unauthorized' }, { status: 401 });
    return NextResponse.json({ success: false, message: 'Failed to update employer status' }, { status: 500 });
  }
}
