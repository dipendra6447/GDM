...
export function AuthPage({ initialIsLogin = true }: { initialIsLogin?: boolean }) {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      if (mode === 'register' || mode === 'signup' || window.location.pathname.startsWith('/register')) {
        setIsLogin(false);
      } else if (mode === 'login' || (window.location.pathname.startsWith('/login') && !role && !mode)) {
        setIsLogin(true);
      }
    ...
    gsap.to(formRef.current, {
      ...
      onComplete: () => {
        setIsLogin(prev => {
          const nextVal = !prev;
          if (typeof window !== 'undefined') {
            const nextPath = nextVal ? '/login' : '/register';
    ...
    <div className="auth-page-wrapper">
      ...
      <div className="auth-split-right">
        <div className="auth-form-container" ref={formRef}>
          ...
          {/* GOOGLE OAUTH BUTTON */}
          <a href={`${process.env.NEXT_PUBLIC_API_URL || ''}/api/auth/google`} className="auth-social-btn">
